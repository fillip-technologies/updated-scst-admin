        ...
        <div class="bg-white rounded-2xl shadow p-6">
            ...
            <form action="{{ route('admin.notice.update',$notice->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="title" value="{{ $notice->title ?? '' }}" placeholder="Enter notice title"
                ...
                <div class="mb-4">
                    ...
                    <select name="notice_type" class="w-full border rounded-lg px-4 py-2">
                        <option value="">Select Category</option>
                        <option value="Admission" {{ ($notice->notice_type ?? '') == 'Admission' ? 'selected' : '' }}>Admission
                        </option>
                        <option value="Events" {{ ($notice->notice_type ?? '') == 'Events' ? 'selected' : '' }}>Events</option>
                        <option value="Academic" {{ ($notice->notice_type ?? '') == 'Academic' ? 'selected' : '' }}>Academic
                        </option>
                ...
                <div class="mb-4 flex items-center gap-2">
                    <input type="checkbox" name="is_new" {{ $notice->is_new ?? false ? 'checked' : '' }}>
                ...
                <div class="mb-4">
                    ...
                    <input type="file" name="file" accept=".pdf" class="w-full border rounded-lg px-4 py-2">
                </div>
